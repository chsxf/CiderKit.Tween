...
public enum LoopingType: Sendable {
    ...
    /// - Parameter loopCount: 0 means looping indefinitely, 1 means no looping (similar to ```none```)
    ///         and any value above means the tween will execute `loopCount` times
    ...
    ///
    /// - Parameter loopCount: 0 means looping indefinitely, 1 means no looping (similar to ```none```)
    ///         and any value above means the tween will execute `loopCount` times (progressing "forward" with odd iterations and "backwards" with even iterations)
